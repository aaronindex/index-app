 ...
 * 
 * This script will:
 * - Delete all conversations, messages, chunks, embeddings
 * - Delete all projects, highlights, tasks, decisions
 * - Delete all tags, themes, digests, imports
 * - Optionally delete the user account itself
 * 
 * Usage:
 *   npx tsx scripts/cleanup-test-data.ts [--delete-account]
 * 
 * Or with email:
...
async function cleanupUserData(userId: string, deleteAccount: boolean = false) {
  ...
  try {
    ...
    const [
      ...
      { count: decisionsCount } = { count: 0 },
      { count: highlightsCount } = { count: 0 },
    ] = await Promise.all([
...
async function main() {
  ...
  if (email) {
    ...
    const { data: authUser } = await supabase.auth.admin.listUsers();
    const user = authUser?.users.find((u) => u.email === inputEmail);
